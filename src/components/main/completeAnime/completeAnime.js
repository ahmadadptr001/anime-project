...
export default function CompleteAnime() {
    ...
    useEffect(() => {
        ...
        const dataAnime = async () => {
            const response = await axios.get("https://api.jikan.moe/v4/anime?status=complete");
                ...
                <div className="card border-0">
                    <div className="card-header bg-dark p-3 border-0">
                        <h2 className="card-title fw-bold fs-2 text-light" style={{fontFamily : "open sans"}}>
                            Anime<span className="text-warning">Tamat</span>Rawwrr
                        </h2>
                    </div>

                    <div className="card-body anime-complete mt-3">
